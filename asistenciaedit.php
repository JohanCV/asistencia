                        ...
                        <table class="table table-sm text-uppercase table-info-asistencia">
                            <tbody>
                                ...
                                <!--tr>
                                  ...
                                  <td>
                                    <input name="tema" class="form-control" type="text" placeholder="Ingrese el tema..."> 
                                  </td>
                                </tr-->
                                <tr>
                                  ...
                                  <td>
                                    <div class="progress progress-md mb-2">
                                      <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width: <?= $rpta?>%" aria-valuenow="6" aria-valuemin="0" aria-valuemax="100">
                                       <?php
                                           echo $rpta.' %'; ?>
                                ...
                                <!--tr>
                                  ...
                                  <th scope="row"></th>                                
                                    <td>
                                      ...
                                      <!--button type="submit" name="guardarCab" class="btn btn-success" id="saveCab"><i class="fas fa-fw fa-check"></i> Guardar</button-->
                                    </td>
                                  </tr>
            ...
            <div class="card shadow mb-4">
              ...
              <div class="card-body">
                ...
                <?php 
                  $_SESSION['asis']=array();
                    echo "<table borde='2' class='table table-bordered asistencia-lista'  id='dataTable' width ='100%' cellspacing='0' >";
                          ...
                          $i=1;
                          while ($columna2 = mysqli_fetch_array( $resultadoAsisEdit )){                                  
                              // echo "<thead>";
                              echo "<tr>";

                                  echo "<td>".$i ."</td>
                                  <td>".utf8_decode($columna2['cui']) ."</td>
                                  <td>".utf8_decode($columna2['nombres']) ."</td>
                                  <td>".utf8_decode($columna2['apellidop'] )."</td>
                                  <td>".utf8_decode($columna2['apellidom'])  ."</td>
                                  <td>".'<div style="display: none;">' .$columna2['estado'].'</div>'."</td>";
                ...
                <div class="text-right">
                   ...
                   <a href=""  class="btn btn-warning" onclick="sendDataToPHP()">Guardar Asistencia </a>
    ...
    $(window).on('load', function() {
      ...
      $(".asistencia-lista tbody tr:not(.asistencia-check)").each(function(e){
        ...
        var html = '<section class="text-center">'+
                  ...
                  if (countChecked > 0) {
                    $(this).find('input[value=T]').prop('checked', true);
                  }

        table.append(html);//("input[name=gender][value=" + dataItem[0].gender + "]").attr('checked', 'checked');
      });
      
      $.when($.getScript("js/svgcheckbx.js") ).done(function () {
        $("#dataTable tr td:last-child").each(function(){
          var asistencia = $(this).text()[0];
          
          if(asistencia == "P"){
            $(this).find("li").eq(0).find("input").trigger("click");
          } 
          else if(asistencia == "T"){
            $(this).find("li").eq(1).find("input").trigger("click");
          } 
          else {
            $(this).find("li").eq(2).find("input").trigger("click");
          }
        });
      }); 
       ...
       function datosAsistencia(){   
        ...
        $('#dataTable tbody tr ').each(function(row, tr){
          ...
          var value;
              ...
              //       data +=", \"observacion\" :\" \"";
              //       data +=", \"fk\" :\""+<?php echo (empty($_SESSION["id_cabecera"][0]))? $_SESSION["idcabeceracontinuo"][0] : $_SESSION["id_cabecera"][0];?>+"\""
              //       +"}," // asistencia          
        arrayData.push(
         TableData[row] ={
